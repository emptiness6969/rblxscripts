...
assert(getrawmetatable, "Exploit unsupported")
assert(Drawing.new, "Exploit unsupported")
assert(setreadonly and make_writeable, "Exploit unsupported")
...
_G.Config.gunModsEnabled = false
_G.Config.infiniteRange = false
_G.Config.infiniteVelocity = false
...
_G.Config.espHeadDotsTransparency = 0
_G.Config.espHeadDotsColor = Color3.new(1, 0, 0)
_G.Config.viewmodelEnabled = false
_G.Config.viewmodelArms = false
_G.Config.viewmodelGloves = false
_G.Config.viewmodelWeapon = false
_G.Config.viewmodelOffsetEnabled = false
_G.Config.viewmodelOffsetX = 0
_G.Config.viewmodelOffsetY = 0
_G.Config.viewmodelOffsetZ = 0

_G.Config.noFallDamage = false

_G.configName = "default"

local mainWindow = library:AddWindow("Blackhawk Rescue Mission 5", {
    main_color = Color3.fromRGB(41, 74, 122),
...
local gunModsTab = mainWindow:AddTab("Gun Mods")
...
local dropdown = aimbotTab:AddDropdown("Hitbox", function(val)
    _G.Config.silentAimHitbox = val
end)
dropdown:Add("Head")
dropdown:Add("HumanoidRootPart")

aimbotTab:AddSlider("FOV", function(val)
    _G.Config.silentAimFov = val
end, {min = 1, max = 800, readonly = false})




gunModsTab:AddSwitch("Enabled", function(bool)
	_G.Config.gunModsEnabled = bool
end)

local gunModsFolder = gunModsTab:AddFolder("Main")

gunModsFolder:AddSwitch("No recoil", function(bool)
    _G.Config.noRecoil = bool
end)

gunModsFolder:AddSwitch("Infinite range", function(bool)
    _G.Config.infiniteRange = bool
end)

gunModsFolder:AddSwitch("Infinite velocity", function(bool)
    _G.Config.infiniteVelocity = bool
end)


visualsTab:AddSwitch("Enabled", function(bool)
	_G.Config.visualsEnabled = bool
end)
...
espFolder:AddSwitch("Show FOV circle", function(bool)
    _G.Config.showFovCircle = bool
end)

espFolder:AddSlider("Head dots transparency", function(val)
    _G.Config.espHeadDotsTransparency = val * 0.1
end, {min = 0, max = 10, readonly = false})

espFolder:AddSlider("FOV circle transparency", function(val)
    _G.Config.fovCircleTransparency = val * 0.1
end, {min = 0, max = 10, readonly = false})

local viewmodelFolder = visualsTab:AddFolder("Viewmodel")

viewmodelFolder:AddSwitch("Enabled", function(bool)
    _G.Config.viewmodelEnabled = bool
end)

viewmodelFolder:AddSwitch("Arms", function(bool)
    _G.Config.viewmodelArms = bool
end)

viewmodelFolder:AddSwitch("Gloves", function(bool)
    _G.Config.viewmodelGloves = bool
end)

viewmodelFolder:AddSwitch("Weapon", function(bool)
    _G.Config.viewmodelWeapon = bool
end)

viewmodelFolder:AddLabel("Viewmodel Offset")

viewmodelFolder:AddSwitch("Enabled", function(bool)
    _G.Config.viewmodelOffsetEnabled = bool
end)

viewmodelFolder:AddSlider("X", function(val)
    _G.Config.viewmodelOffsetX = val * 0.5
end, {min = 0, max = 360, readonly = false})

viewmodelFolder:AddSlider("Y", function(val)
    _G.Config.viewmodelOffsetY = val * 0.5
end, {min = 0, max = 360, readonly = false})

viewmodelFolder:AddSlider("Z", function(val)
    _G.Config.viewmodelOffsetZ = val * 0.5
end, {min = 0, max = 360, readonly = false})

Misc:AddSwitch("No fall damage", function(bool)
	_G.Config.noFallDamage = bool
end)

local configsFolder = Misc:AddFolder("Configs")

configsFolder:AddTextBox("config name", function(val)
	_G.configName = val
end)
configsFolder:AddButton("Load", function()
	if readfile(_G.configName .. ".BRM5") then
		local table = game:GetService("HttpService"):JSONDecode(readfile(_G.configName .. ".BRM5"))
...
configsFolder:AddButton("Save", function()
	writefile(_G.configName .. ".BRM5", game:GetService("HttpService"):JSONEncode(_G.Config))
end)
...
function playerAlive(player, healthCheck)
	if player.Character and player.Character:FindFirstChildOfClass("Humanoid") and player.Character:FindFirstChild("HumanoidRootPart") then
		if (healthCheck and player.Character:FindFirstChildOfClass("Humanoid").Health > 0) or not healthCheck then
			return true
		end
	end
	...
	if redo then
		if objectOverrideData[object] then
			for i,v in pairs(objectOverrideData[object]) do
				...
				object[v["property"]] = v["val"]
			end
		end
	else
		if not objectOverrideData[object] then

			local t = copyTable(table)
			for i,v in pairs(t) do
				v["val"] = object[v["property"]]
			end
			objectOverrideData[object] = t
		end
		...
		if rawget(v, "FireServer") and rawget(v, "InvokeServer") then
			...
			v.FireServer = function(self, ...)
				...
				elseif args[1] == "banUser" then
					return
				elseif args[1] == "replicateCharacter" then
					args[2]._packet = -9e9
					args[3]._metadata = -9e9
				end
			
				return __oldFireServer(self, unpack(args))
			end
		end
	end
end
...
local __oldTakeDamage
__oldTakeDamage = hookfunction(Instance.new("Humanoid").TakeDamage, function(...)
	if _G.Config.noFallDamage then
		return
	end
	return __oldTakeDamage(...)
end)

spawn(function()
	while true do
		if _G.Config.gunModsEnabled then
			for i,v in pairs(getgc(true)) do
        ...
        if _G.Config.visualsEnabled and _G.Config.viewmodelEnabled then
			if playerAlive(localPlayer, true) and localPlayer.Character:FindFirstChild("RA") and localPlayer.Character:FindFirstChild("LA") then
				local weaponsOverride = {
					{
						property = "Material",
						val = Enum.Material.ForceField
					},
					{
						property = "Color",
						val = Color3.fromHSV(tick() % 5 / 5, 1, 1)
					}
				...
				local armsOverride = {
					...
					{
						property = "Color",
						val = Color3.new(0.9, 0, 0)
					}
					
				}
				local glovesOverride = {
					{
						property = "Material",
						val = Enum.Material.ForceField
					},
				...
				if _G.Config.viewmodelArms then
					overrideObject(localPlayer.Character.RA.Base, armsOverride, false)
					overrideObject(localPlayer.Character.LA.Base, armsOverride, false)
				else
					overrideObject(localPlayer.Character.RA.Base, nil, true)
					overrideObject(localPlayer.Character.LA.Base, nil, true)
				end
				if _G.Config.viewmodelGloves then
					overrideObject(localPlayer.Character.RA.Glove, glovesOverride, false)
					overrideObject(localPlayer.Character.LA.Glove, glovesOverride, false)
				else
					overrideObject(localPlayer.Character.RA.Glove, nil, true)
					overrideObject(localPlayer.Character.LA.Glove, nil, true)
				end
				if _G.Config.viewmodelWeapon then
					for i,v in pairs(localPlayer.Character:GetChildren()) do
						if v:FindFirstChild("barrel") then
							for i2, v2 in pairs(v:GetDescendants()) do
								if v2.ClassName == "MeshPart" then
									overrideObject(v2, weaponsOverride, false)
								end
							end
						end
					end
			...
			if playerAlive(localPlayer, true) and localPlayer.Character:FindFirstChild("RA") and localPlayer.Character:FindFirstChild("LA") then
				overrideObject(localPlayer.Character.RA.Base, armsOverride, true)
				overrideObject(localPlayer.Character.LA.Base, armsOverride, true)
				overrideObject(localPlayer.Character.RA.Glove, glovesOverride, true)
				overrideObject(localPlayer.Character.LA.Glove, glovesOverride, true)
				for i,v in pairs(localPlayer.Character:GetChildren()) do
					if v:FindFirstChild("barrel") then
						for i2, v2 in pairs(v:GetDescendants()) do
							if v2.ClassName == "MeshPart" then
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if getnamecallmethod() == "FindPartOnRayWithIgnoreList" and string.find(debug.traceback(), "FirearmInventory:353") and _G.Config.silentAimEnabled and closestPlayer then
        args[1] = Ray.new(Camera.CFrame.p, (closestPlayer.Character[_G.Config.silentAimHitbox].Position - Camera.CFrame.p).unit * 69420)
	elseif getnamecallmethod() == "TakeDamage" and _G.Config.noFallDamage then
		return
    end
		...
		if getinfo(v).numparams == 5 then
		    ...
		    if a[1] == "ToWorldSpace" and a[2] == "ToObjectSpace" and a[3] == "WorldCFrame" then
		        ...
		        __oldViewmodel = hookfunction(v, function(...)
					...
					if _G.Config.viewmodelOffsetEnabled then
